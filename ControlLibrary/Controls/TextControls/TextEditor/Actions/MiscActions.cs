using ControlLibrary.Controls.TextControl.TextEditor.Document;
using System;
using System.Diagnostics;
using System.Text;

namespace ControlLibrary.Controls.TextControl.TextEditor.Actions
{
	...
	public class ShiftTab : AbstractEditAction
	{
		...
		public override void Execute(TextArea textArea)
		{
			...
			else
			{
				...
				_ = textArea.Document.GetText(line.Offset, textArea.Caret.Offset - line.Offset);
	...
	public class ToggleLineComment : AbstractEditAction
	{
		...
		private void SetCommentAt(IDocument document, string comment, ISelection selection, int y1, int y2)
		{
			...
			for (int i = y2; i >= y1; --i)
			{
				...
				_ = document.GetText(line.Offset, line.Length);
			...
			else
			{
				if (textArea.Caret.Offset > 0 && !textArea.IsReadOnly(textArea.Caret.Offset - 1))
				{
					...
					if (curLineOffset == textArea.Caret.Offset)
					{
						...
						_ = curLineNr == textArea.Document.TotalNumberOfLines;
